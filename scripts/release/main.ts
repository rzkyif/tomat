...
import { main as runCatalog } from "./catalog.ts";
...
async function main(): Promise<void> {
  ...
  await runCore();
  step("RELEASE 2/7: toolkit");
  await runToolkit();
  step("RELEASE 3/7: catalog");
  await runCatalog();
  step("RELEASE 4/7: client");
  await runClient();
  step("RELEASE 5/7: install scripts");
  await runScripts();
  step("RELEASE 6/7: schemas");
  await runSchemas();
  step("RELEASE 7/7: website");
  await runWebsite();
