// Collect structured rows into the toolkit's private database: the model
...
// kind and the per-toolkit database.
...
export async function collectTable(
  ...
  const columns = (Array.isArray(args?.columns) ? args.columns : [])
    ...
    .map((c) => c.trim());
  if (columns.length === 0) throw new Error("columns must name at least one column");
