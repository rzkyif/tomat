...
export const toolFilterPhase1EntrySchema = z.object({
...
export const toolFilterEntrySchema = z.object({
  toolId: z.string().min(1),
  name: z.string(),
  description: z.string(),
}).passthrough();
...
const chatToolfilterFrameSchema = z.object({
...
const chatChunkFrameSchema = z.object({
...
const chatToolcallRequestedFrameSchema = z.object({
...
const chatUsageFrameSchema = z.object({
...
const chatErrorFrameSchema = z.object({
...
const toolAskuserRequestFrameSchema = z.object({
...
const toolLogFrameSchema = z.object({
...
const toolErrorFrameSchema = z.object({
...
const toolCancelledFrameSchema = z.object({
  kind: z.literal("tool.cancelled"),
  callId: z.string().min(1),
}).passthrough();

const toolkitInstallLogFrameSchema = z.object({
...
const toolkitInstallDoneFrameSchema = z.object({
...
const toolkitSnapshotFrameSchema = z.object({
  kind: z.literal("toolkit.snapshot"),
}).passthrough();

// DownloadEntry/SidecarSnapshot are large domain shapes — use z.unknown()
// in the payload so the WS envelope passes without us copying every
// nested field. Downstream `downloads.svelte.ts` / `sidecar.svelte.ts`
// own the real validation against their domain types.
const downloadsSnapshotFrameSchema = z.object({
  kind: z.literal("downloads.snapshot"),
  items: z.array(z.unknown()),
}).passthrough();

const sidecarStatusFrameSchema = z.object({
...
const sessionUpdatedFrameSchema = z.object({
...
const updateStagedFrameSchema = z.object({
  kind: z.literal("update.staged"),
  version: z.string(),
}).passthrough();

const updateErrorFrameSchema = z.object({
...
export type ServerToClientFrameParsed = z.infer<
  typeof serverToClientFrameSchema
>;
