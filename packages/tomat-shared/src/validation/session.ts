...
const toolFilterPhase1Schema = z.object({
  toolId: z.string(),
  name: z.string(),
...
const toolFilterEntrySchema = z.object({
  toolId: z.string(),
  name: z.string(),
  description: z.string(),
}).strict();
...
export const systemMessageInputSchema = z.object({
  ...baseShape,
  role: z.literal("system"),
  content: z.string(),
}).strict();

export const toolMessageInputSchema = z.object({
  ...baseShape,
  role: z.literal("tool"),
  callId: z.string(),
  toolkitId: z.string(),
...
export const reasoningMessageInputSchema = z.object({
  ...baseShape,
...
export const toolFilterMessageInputSchema = z.object({
  ...baseShape,
...
export const errorMessageInputSchema = z.object({
  ...baseShape,
...
export const messagePatchSchemaByRole = {
  ...
  assistant: assistantMessageInputSchema.omit({ role: true }).partial()
    .strict(),
  system: systemMessageInputSchema.omit({ role: true }).partial().strict(),
  tool: toolMessageInputSchema.omit({ role: true }).partial().strict(),
  reasoning: reasoningMessageInputSchema.omit({ role: true }).partial()
    .strict(),
  tool_filter: toolFilterMessageInputSchema.omit({ role: true }).partial()
    .strict(),
