<script lang="ts">
  ...
  import type { SettingTab } from "@tomat/shared";

  // Segmented tab selector rendered like Toggle's labelled switch: a
  // bg-surface-inset groove with every tab label resting on it and a knob that
  // slides to the active tab. The knob is an aria-hidden overlay clipped to the
  // active cell with clip-path: inset(); transitioning the clip rect slides it
  // and repaints each label exactly where the knob edge crosses. Differences
  // from Toggle: labels are normal case, and the knob is a subtle raised shade
  // (bg-surface-inset-strong, lighter than the groove in dark mode) rather than
  ...
  const knobStyle = $derived.by(() => {
    ...
    return `clip-path: ${clip}; transition: clip-path ${slideMs ?? getDuration()}ms ${CSS_EASING};`;
  });
