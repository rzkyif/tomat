...
// data/signing-keys.json; regenerated by `deno task release`).
//
...
export async function loadBinaryManifest(opts: FetchOptions = {}): Promise<BinaryManifest> {
  ...
  // LATEST upstream sidecar release at install/update time, exactly like latest.
 ...
 *  the config is part of this from-source build). Same shape a latest release
...
function devManifest(): BinaryManifest {
  ...
  for (const kind of BINARY_KINDS) {
    ...
    // Self-hosted kinds (tomat-core-speech) have no upstream resolver; the dev
    // flow builds + stages them from source (scripts/dev.ts), so they need no
    // manifest entry to resolve at runtime.
