...
// signing-keys.ts; regenerated by `deno task website:deploy`).
//
...
import { BINARY_MANIFEST_URL } from "../config.ts";
import { paths } from "../paths.ts";
import { AppError } from "../shared/errors.ts";
import { MANIFEST_PUBLIC_KEY_B64 } from "../signing-keys.ts";
...
async function fetchAndVerify(signal?: AbortSignal): Promise<BinaryManifest> {
  let res: Response;
  try {
    res = await fetch(BINARY_MANIFEST_URL, { signal });
  } catch (err) {
    throw new AppError(
      "manifest_fetch_failed",
      `failed to fetch binary manifest: ${
...
async function verifyManifestSignature(m: BinaryManifest): Promise<void> {
  const pk = decodeBase64(MANIFEST_PUBLIC_KEY_B64);
