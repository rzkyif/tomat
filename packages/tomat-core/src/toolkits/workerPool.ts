...
import { WorkerHandle } from "./workerHandle.ts";
...
export class WorkerPool {
  ...
  private evictLeastRecent(): void {
    const lru = this.lru[0];
    if (!lru) return;
    const w = this.workers.get(lru);
    if (!w) return;
    if (w.inFlightCalls > 0) {
      // Don't evict a worker mid-call; bump the LRU and try the next one.
      this.touchLru(lru);
      return;
    }
    log.info(`evicting idle worker for ${lru}`);
    void w.terminate(this.config.drainTimeoutMs);
    this.workers.delete(lru);
    this.removeFromLru(lru);
  }
