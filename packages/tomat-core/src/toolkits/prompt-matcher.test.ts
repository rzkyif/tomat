...
const templates: PathTemplates = {
  ...
  toolkit: "/Users/u/.tomat/core/toolkits/tk",
};
...
Deno.test("matcher: declared ask-state net permission forwards", () => {
  const decl: PermissionDecl = { kind: "net", host: "api.example.com", ports: [443], reason: "r" };
  const d = decidePrompt(
    { permission: "net", resource: "api.example.com:443" },
    ctx([decl], [grant(decl, "ask")]),
  );
  assertEquals(d, { action: "forward", declared: true, reason: "r", permissionKind: "net" });
});
...
Deno.test("matcher: net wildcard host and port", () => {
  const decl: PermissionDecl = { kind: "net", host: "*", ports: ["*"], reason: "any" };
...
Deno.test("matcher: read path template expands and prefix-matches", () => {
  const decl: PermissionDecl = { kind: "read", path: "$downloads", reason: "dl" };
...
Deno.test("matcher: run matches resolved absolute path by basename", () => {
  const decl: PermissionDecl = { kind: "run", binary: "ffmpeg", reason: "encode" };
  const d = decidePrompt({ permission: "run", resource: "/opt/homebrew/bin/ffmpeg" }, ctx([decl]));
  assertEquals(d?.declared, true);
});
...
Deno.test("matcher: path-pinned run declaration does not match same name elsewhere", () => {
  ...
  const decl: PermissionDecl = { kind: "run", binary: "/usr/bin/git", reason: "vcs" };
  const exact = decidePrompt({ permission: "run", resource: "/usr/bin/git" }, ctx([decl]));
  assertEquals(exact?.declared, true);
  const imposter = decidePrompt({ permission: "run", resource: "/tmp/evil/git" }, ctx([decl]));
  // Falls through to undeclared (default policy deny).
  assertEquals(imposter, { action: "deny", declared: false, permissionKind: "run" });
});
...
Deno.test("matcher: undeclared access follows toolkit policy", () => {
  const denied = decidePrompt({ permission: "read", resource: "/secret" }, ctx([], [], "deny"));
  assertEquals(denied, { action: "deny", declared: false, permissionKind: "read" });
  const asked = decidePrompt({ permission: "read", resource: "/secret" }, ctx([], [], "ask"));
  assertEquals(asked, { action: "forward", declared: false, permissionKind: "read" });
});
