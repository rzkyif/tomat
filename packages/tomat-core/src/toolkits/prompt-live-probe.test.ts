...
async function runProbe(scriptPath: string): Promise<void> {
  ...
  const scheduleAnswer = () => {
    ...
    const tick = () => {
      ...
      void writeControl({ kind: "write", dataB64: encodeBase64(pendingAnswer) });
  ...
  const stdoutDone = (async () => {
    ...
    for await (const chunk of proc.stdout) {
      ...
      for (const line of lines) {
        if (!line.trim()) continue;
        const o = JSON.parse(line) as { label: string; ok?: boolean; name?: string };
        if (o.label !== "end") results.set(o.label, { ok: o.ok === true, name: o.name });
      }
    }
  })();
