...
// permissions + grants and the toolkit's undeclared-access policy.
//
...
// permission follows the toolkit's undeclared policy. A prompt CAN match a
...
export function decidePrompt(
  ...
  // Prompt kinds outside the tools.json vocabulary (e.g. "import") cannot be
  ...
  if (state === "denied") {
    return { action: "deny", declared: true, reason: decl.reason, permissionKind: kind };
  }
  return { action: "forward", declared: true, reason: decl.reason, permissionKind: kind };
}
