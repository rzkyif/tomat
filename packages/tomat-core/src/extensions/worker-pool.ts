...
export interface ToolCallStart {
  extensionId: string;
  tool: Tool;
  // tomat.json-declared required permissions for this tool (must already
...
export class WorkerPool {
  ...
  startCall(spec: ToolCallStart, onEvent: (event: CallEvent) => void): CallController {
    ...
    const endUserWait = () => {
      ...
      if (pendingUserWaits === 0 && timeout === undefined && !cancelled) {
        armTimeout();
      }
    };
      ...
      const off = worker.on((frame) => {
        ...
        switch (frame.kind) {
          ...
          case "ask_user_request": {
            ...
            if (!questionsValid) {
              log.warn(
                `invalid ask_user_request from ${spec.extensionId}/${spec.tool.name}; answering empty`,
              );
              onEvent({
                kind: "log",
                level: "warn",
          ...
          case "schedule_request": {
            ...
            if (!parsedDraft.success) {
              log.warn(
                `invalid schedule_request draft from ${spec.extensionId}/${spec.tool.name}; rejecting`,
              );
              onEvent({
                kind: "log",
                level: "warn",
          ...
          case "display": {
            ...
            if ("error" in bounded) {
              log.warn(
                `dropping display from ${spec.extensionId}/${spec.tool.name}: ${bounded.error}`,
              );
              onEvent({
                kind: "log",
                level: "warn",
              ...
              .then(
                (result) => {
                  try {
                    ...
                    log.warn(
                      `module_response for ${spec.extensionId}/${spec.tool.name} not serializable: ${errMessage(
                        err,
                      )}`,
                    );
...
function resolveEntryPath(extensionFolder: string): string {
